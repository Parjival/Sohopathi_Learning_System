...
exports.createAudiobook = async (req, res, next) => {
  try {
    ...
    const payload = { content, title, voice_preference };
    const result = await callSkill('/create_audiobook', payload);
    const resource = await Resource.create({ owner: req.user._id, type: 'audiobook', title: title||'Audiobook', meta: result.meta||{}, smythosId: result.id, storageUrl: result.url });
